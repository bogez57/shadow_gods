...
class RunTimeArr
{
    ...
    {
        BGZ_ASSERT(index < capacity, "RunTimeArr: Attempting to access index %i which is out of current arround bounds - current max index allowed: %i", index, capacity - 1);
        BGZ_ASSERT(index < length, "RunTimeArr: Attempting to access index %i which hasn't been initialized yet. Current buffer length is %i", index, length);
    ...
    inline Type& Push()
    {
        BGZ_ASSERT(length < capacity, "RunTimeArr: Attempting to access index %i which out of current arround bounds - current max index allowed: %i", length, capacity - 1);
...
void CopyArray(RunTimeArr<Type> sourceArray, RunTimeArr<Type>&& destinationArray)
{
    ...
    memcpy(destinationArray.elements, sourceArray.elements, sizeof(Type) * sourceArray.capacity);
