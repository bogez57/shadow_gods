...
/**
 * Creates a newNode empty list and returns a status code.
 *
 ...
 * the memory allocation for the newNode List failed.
 */
...
/**
 * Creates a newNode empty list based on the specified ListConf struct and
 ...
 * memory allocation for the newNode List structure failed.
 */
enum cc_stat list_new_conf(ListConf const * const conf, List **out)
{
    List *list = (List*)conf->mem_calloc(1, sizeof(List));
...
/**
 * Adds a newNode element to the list. The element is appended to the list making it
 ...
 * memory allocation for the newNode element failed.
 */
...
/**
 * Prepends a newNode element to the list (adds a newNode "head") making it the first
 * element of the list.
 *
 * @param[in] list list to which the element is being added
...
{
    Node *node = (Node*)list->mem_calloc(1, sizeof(Node));
...
/**
 * Appends a newNode element to the list (adds an newNode "tail") making it the last
 * element of the list.
 *
 * @param[in] list list to which the element is being added
...
/**
 * Adds a newNode element at the specified location in the List and shifts all
 * subsequent elements by one. This operation cannot be performed on an empty
 * List. The index at which the newNode element is being added must be within the
 * bounds of the List.
 *
 * @param[in] list list to which this element is being added
 * @param[in] element element that is being added
 * @param[in] index the position in the list at which the newNode element is being
 *                  added
 *
 ...
 * allocation for the newNode element failed.
 */
enum cc_stat list_add_at(List *list, void *element, size_t index)
{
    ...
    Node *newNode = (Node*)list->mem_calloc(1, sizeof(Node));

    if (!newNode)
        return CC_ERR_ALLOC;

    newNode->data = element;
    link_behind(base, newNode);
 ...
 * the memory allocation for the newNode elements failed.
 */
 ...
 * CC_ERR_ALLOC if the memory allocation for the newNode elements failed.
 */
enum cc_stat list_add_all_at(List *list1, List *list2, size_t index)
{
    ...
    /* Link the newNode nodes together outside of the list so
    ...
    /* Now we can safely attach the newNode nodes. */
    Node *end = NULL;
 ...
 * @param[in, out] h the pointer to which the newNode head will be attached
 * @param[in, out] t the pointer to which the newNode tail will be attached
 *
 ...
 * elements [5, 6, 7, 8, 9], a sublist from index 1 to 3 will will be a newNode
 ...
 * @param[out] out Pointer to where the newNode sublist is stored.
 *
 ...
 * for the newNode sublist failed.
 */
 ...
 * holds.
 *
 * @note The newNode list is allocated using the original lists allocators and also
 ...
 * list is emtpy, or CC_ERR_ALLOC if the memory allocation for the newNode array failed.
 */
enum cc_stat list_to_array(List *list, void ***out)
{
    ...
    void **array = (void**)list->mem_calloc(list->size, sizeof(void*));
    ...
    for (i = 0; i < size; i++) {
        ...
        if ((c < 0 || c == 0)) {
            ...
            if (r == r_size) {
                 ...
                 * the rightmost element as the newNode head. */
                for (;l < l_size - 1; l++)
                    l_part = l_part->next;
                *right = l_part;
                break;
...
/**
 * Filters the List by creating a newNode List that contains all elements from the
 ...
 * @param[out] out pointer to where the newNode filtered list is to be stored
 *
...
/**
 * Adds a newNode element to the list after the last returned element by
 ...
 * if the memory allocation for the newNode element failed.
 */
enum cc_stat list_iter_add(ListIter *iter, void *element)
{
    Node *new_node = (Node*)iter->list->mem_calloc(1, sizeof(Node));
...
/**
 * Returns a newNode descending iterator. A descending iterator or a reverse
...
/**
 * Adds a newNode element pair to the lists after the last returned element pair by
...
{
    Node *new_node1 = (Node*)iter->l1->mem_calloc(1, sizeof(Node));

    if (!new_node1)
        return CC_ERR_ALLOC;

    Node *new_node2 = (Node*)iter->l2->mem_calloc(1, sizeof(Node));
