...
local_func void
DrawTexture_UnOptimized(ui32* colorBufferData, v2i colorBufferSize, i32 colorBufferPitch, Quadf targetRect_screenCoords, RenderEntry_Texture texture, Rectf clipRect, Circle areaOfVectorAdjustments)
{
        ...
        for (f32 screenX = xMin; screenX < xMax; ++screenX)
        {
            ...
            if (u >= 0.0f && u <= 1.0f && v >= 0.0f && v <= 1.0f)
            {
                ...
                if (normalMap.mapData)
                {
                    ...
                    f32 yPosSquared = texelPos_y * texelPos_y;
                    f32 xPosSquared = texelPos_x * texelPos_x;
...
void DoRenderWork(void* data)
{
    ...
    for (i32 entryNumber = 0; entryNumber < work->renderingInfo.cmdBuffer.entryCount; ++entryNumber)
    {
        ...
        case EntryType_Texture: {
            ...
            Quadf imageTargetRect_camera = CameraTransform(textureEntry.targetRect_worldCoords, *camera);
            Quadf imageTargetRect_screen = ProjectionTransform_Ortho(imageTargetRect_camera, pixelsPerMeter);
            textureEntry.areaWhereVectorAdjustmentsShouldBeMade.centerPos = textureEntry.areaWhereVectorAdjustmentsShouldBeMade.centerPos * pixelsPerMeter;

            DrawTexture_UnOptimized((ui32*)work->colorBufferData, work->colorBufferSize, work->colorBufferPitch, imageTargetRect_screen, textureEntry, work->screenRegionCoords, textureEntry.areaWhereVectorAdjustmentsShouldBeMade);
